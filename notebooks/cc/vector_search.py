...
class CertProcessor:
    ...
    def process_cert(self, cert: object) -> None:
...
def query_similar_chunks(query_text: str, processor: CertProcessor, k: int = 5) -> list[str]:
    """
...
def check_certificate_count(dset: dict) -> int:
...
def check_txt_files(path_dset: Path) -> int:
    ...
    txt_paths = [
        path_dset / "certs/certificates/txt",
        path_dset / "certs/reports/txt",
        path_dset / "certs/targets/txt"
    ]
...
def manage_database(db_path: Path, model: SentenceTransformer, force_rebuild: bool, dset: dict) -> bool:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="../../cc_data_gemini",
                        help="Path to the dataset directory")
    parser.add_argument("--db-path", type=str, default="vector_db.sqlite",
                        help="Path to the SQLite database file")
    parser.add_argument("--force-rebuild", action="store_true",
                        help="Force database rebuild even if it exists")
