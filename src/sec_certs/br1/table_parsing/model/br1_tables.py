...
from .entry_types.auth import *
from .entry_types.err_states import *
from .entry_types.modes_of_op import *
from .entry_types.module_id import *
from .entry_types.physical_sec import *
from .entry_types.ports import *
from .entry_types.sec_levels import *
from .entry_types.self_tests import *
from .entry_types.services import *
from .entry_types.ssp import *
...
class BR1TablesClass:
    # Security Levels
    security_levels: BR1Table[SecurityLevel] = field(
        default_factory=lambda: BR1Table("", 1, 2, SecurityLevel)
    )
    ...
    # Modes of Operation
    modes_of_operation: BR1Table[ModeOfOp] = field(
        default_factory=lambda: BR1Table("", 2, 4, ModeOfOp)
    )
    ...
    ports_interfaces: BR1Table[PortInterface] = field(
        default_factory=lambda: BR1Table("", 3, 1, PortInterface)
    )

    # Authentication Methods
    authentication_methods: BR1Table[AuthMethod] = field(
        default_factory=lambda: BR1Table("", 4, 1, AuthMethod)
    )
    roles: BR1Table[Role] = field(default_factory=lambda: BR1Table("", 4, 2, Role))

    # Services
    approved_services: BR1Table[ApprovedService] = field(
        default_factory=lambda: BR1Table("", 4, 3, ApprovedService)
    )
    ...
    # Physical Security
    mechanisms_actions: BR1Table[PhSecMechanism] = field(
        default_factory=lambda: BR1Table("", 7, 1, PhSecMechanism)
    )

    # SSPs
    storage_areas: BR1Table[StorageArea] = field(
        default_factory=lambda: BR1Table("", 9, 1, StorageArea)
    )

    ssp_io_methods: BR1Table[SspIOMethod] = field(
        default_factory=lambda: BR1Table("", 9, 2, SspIOMethod)
    )
    ...
    # Self Tests
    self_tests: BR1Table[SelfTest] = field(
        default_factory=lambda: BR1Table("", 10, 1, SelfTest)
    )
    cond_self_tests: BR1Table[CondSelfTest] = field(
        default_factory=lambda: BR1Table("", 10, 2, CondSelfTest)
    )

    # Error States
    error_states: BR1Table[ErrorState] = field(
        default_factory=lambda: BR1Table("", 10, 4, ErrorState)
    )
