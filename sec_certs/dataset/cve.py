...
class CVEDataset(ComplexSerializableType):
    ...
    def build_lookup_dict(
        ...
        nist_matching_filepath: Optional[Path] = None,
    ):
        """
        ...
        for cve in helpers.tqdm(
            self, desc="Building-up lookup dictionaries for fast CVE matching"
        ):
    ...
    def download_cves(cls, output_path_str: str, start_year: int, end_year: int):
        ...
        urls = [
            cls.cve_url + str(x) + ".json.zip" for x in range(start_year, end_year + 1)
        ]

        logger.info(
            f"Identified {len(urls)} CVE files to fetch from nist.gov. Downloading them into {output_path}"
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            outpaths = [Path(tmp_dir) / Path(x).name.rstrip(".zip") for x in urls]
            responses = list(
                zip(
                    *helpers.download_parallel(
                        list(zip(urls, outpaths)), num_threads=config.n_threads
                    )
                )
            )[1]
    ...
    @classmethod
    def from_web(
        cls, start_year: int = 2002, end_year: int = datetime.datetime.now().year
    ):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            all_cves = dict()
            logger.info(
                f"Downloaded required resources. Building CVEDataset from jsons."
            )
    ...
    def filter_related_cpes(self, relevant_cpes: Set[CPE]):
        ...
        for cve in self:
            ...
            cve.vulnerable_cpes = list(
                filter(lambda x: x in relevant_cpes, cve.vulnerable_cpes)
            )
    ...
    def get_nist_cpe_matching_dict(self, input_filepath: Optional[Path]):
        def parse_key_cpe(field: Dict) -> CPE:
            ...
            return CPE(
                field["cpe23Uri"], start_version=start_version, end_version=end_version
            )
        ...
        if not input_filepath or not input_filepath.is_file():
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                if input_filepath:
                    logger.debug(
                        f"Copying attained NIST mapping file to {input_filepath}"
                    )
        ...
        mapping_dict = dict()
        for match in helpers.tqdm(
            match_data["matches"], desc="parsing cpe matching (by NIST) dictionary"
        ):
