...
class Dataset(ABC):
    ...
    def _download_parallel(
        urls: Collection[str], paths: Collection[Path], prune_corrupted: bool = True
    ):
        ...
        logger.info(
            f"Successfully downloaded {n_successful} files, {len(exit_codes) - n_successful} failed."
        )
    ...
    def _prepare_cve_dataset(
        ...
        cve_dataset.build_lookup_dict(
            use_nist_cpe_matching_dict, self.nist_cve_cpe_matching_dset_path
        )
        return cve_dataset
    ...
    def _compute_cpe_matches(
    ...
    ) -> Tuple[CPEClassifier, CPEDataset]:
        ...
        for cert in helpers.tqdm(
            self, desc="Predicting CPE matches with the classifier"
        ):
    ...
    def to_label_studio_json(self, output_path: Union[str, Path]):
        ...
        for cert in [x for x in self if x.heuristics.cpe_matches]:
            ...
            candidates += ["No good match"] * (
                config.cc_cpe_max_matches - len(candidates)
            )
    ...
    def load_label_studio_labels(self, input_path: Union[str, Path]):
        ...
        logger.info(
            "Translating label studio matches into their CPE representations and assigning to certificates."
        )
        for annotation in helpers.tqdm(
            [x for x in data if "verified_cpe_match" in x],
            desc="Translating label studio matches",
        ):
            match_keys = annotation["verified_cpe_match"]
            match_keys = (
                [match_keys] if isinstance(match_keys, str) else match_keys["choices"]
            )
            match_keys = [x.lstrip("$") for x in match_keys]
            predicted_annotations = [
                annotation[x] for x in match_keys if annotation[x] != "No good match"
            ]
            ...
            for c in certs:
                c.heuristics.verified_cpe_matches = (
                    {x.uri for x in cpes if x is not None} if cpes else None
                )
    ...
    def compute_related_cves(
        ...
        relevant_cpes = set(
            itertools.chain.from_iterable(
                [x.heuristics.cpe_matches for x in cpe_rich_certs]
            )
        )
        ...
        n_vulnerabilities = sum(
            [
                len(x.heuristics.related_cves)
                for x in cpe_rich_certs
                if x.heuristics.related_cves
            ]
        )
        logger.info(
