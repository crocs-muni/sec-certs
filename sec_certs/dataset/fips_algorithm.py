...
class FIPSAlgorithmDataset(Dataset, ComplexSerializableType):
    ...
    def get_certs_from_web(self):
        ...
        helpers.download_file(
            constants.FIPS_ALG_URL + "1", self.root_dir / "page1.html"
        )
    ...
    def _extract_algorithm_information(elements, vendor, date, product, validation):
        for elem in elements:
            ...
            for attachment in attachments:
                product = (
                    elem.text.strip() if "product-name" in attachment["id"] else product
                )
                validation = (
                    elem.text.strip()
    ...
    def parse_html(self):
        ...
        for f in helpers.search_files(self.root_dir):
            ...
            table = html_soup.find(
                "table",
                class_="table table-condensed publications-table table-bordered",
            )
            ...
            for tr in tbody_contents:
                ...
                (
                    ...
                    product,
                    validation,
                ) = FIPSAlgorithmDataset._extract_algorithm_information(
