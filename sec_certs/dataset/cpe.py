...
class CPEDataset(ComplexSerializableType):
    ...
    cpe_url: ClassVar[str] = (
        "https://nvd.nist.gov/feeds/xml/cpe/dictionary/" + cpe_xml_basename + ".zip"
    )
    ...
    @serialize
    def enhance_with_cpes_from_cve_dataset(
        self, cve_dset: Union[CVEDataset, str, Path]
    ):
        ...
        all_cpes_in_cve_dset = set(
            itertools.chain.from_iterable([cve.vulnerable_cpes for cve in cve_dset])
        )

        old_len = len(self.cpes)

        for cpe in helpers.tqdm(
            all_cpes_in_cve_dset, desc="Enriching CPE dataset with new CPEs"
        ):
            if cpe not in self:
                self[cpe.uri] = cpe

        logger.info(
            f"Enriched the CPE dataset with {len(self.cpes) - old_len} new CPE records."
        )
