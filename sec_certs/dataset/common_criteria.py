...
class CCDataset(Dataset, ComplexSerializableType):
    ...
    def to_pandas(self):
        df = pd.DataFrame(
            ...
            columns=CommonCriteriaCert.pandas_columns,
        )
        df = df.set_index("dgst")

        df.not_valid_before = pd.to_datetime(
            df.not_valid_before, infer_datetime_format=True
        )
        df.not_valid_after = pd.to_datetime(
            df.not_valid_after, infer_datetime_format=True
        )
        df = df.astype(
            {"category": "category", "status": "category", "scheme": "category"}
        )
    ...
    def root_dir(self, new_dir: Union[str, Path]):
        ...
        if self.state and old_dset.root_dir != Path(".."):
            logger.info(
                f"Changing root dir of partially processed dataset. All contents will get copied to {new_dir}"
            )
    ...
    def copy_dataset_contents(self, old_dset: "CCDataset"):
        ...
        if old_dset.state.certs_analyzed:
            try:
                shutil.copytree(
                    old_dset.auxillary_datasets_dir, self.auxillary_datasets_dir
                )
            except FileNotFoundError as e:
                logger.warning(f"Attempted to copy non-existing file: {e}")
    ...
    CSV_PRODUCTS_URL = {
        ...
        "cc_products_archived.csv": BASE_URL
        + "/products/certified_products-archived.csv",
    }
    PP_URL = {
    ...
    def active_html_tuples(self) -> List[Tuple[str, Path]]:
        return [
            (x, self.web_dir / y)
            for y, x in self.HTML_PRODUCTS_URL.items()
            if "active" in y
        ]
    ...
    def active_csv_tuples(self) -> List[Tuple[str, Path]]:
        return [
            (x, self.web_dir / y)
            for y, x in self.CSV_PRODUCTS_URL.items()
            if "active" in y
        ]
    ...
    def _merge_certs(
        self, certs: Dict[str, "CommonCriteriaCert"], cert_source: Optional[str] = None
    ):
        """
        ...
        logger.info(
            f"Added {len(new_certs)} new and merged further {len(certs_to_merge)} certificates to the dataset."
        )

    def download_csv_html_resources(
    ...
    def process_protection_profiles(
        ...
        for cert in self:
            ...
            cert.protection_profiles = {
                pp_dataset.pps.get((x.pp_name, x.pp_link), x)
                for x in cert.protection_profiles
            }
    ...
    def _get_all_certs_from_csv(
        self, get_active: bool, get_archived: bool
    ...
    def _parse_single_csv(file: Path) -> Dict[str, "CommonCriteriaCert"]:
        ...
        df = pd.read_csv(
            file, engine="python", encoding="windows-1252", error_bad_lines=False
        )
        ...
        df_main.maintenance_report_link = df_main.maintenance_report_link.map(
            map_ip_to_hostname
        )
        df_main.maintenance_st_link = df_main.maintenance_st_link.map(
            map_ip_to_hostname
        )
        ...
        profiles = {
            x.dgst: set(
                [
                    ProtectionProfile(y)
                    for y in helpers.sanitize_protection_profiles(x.protection_profiles)
                ]
            )
    ...
    def _parse_single_html(file: Path) -> Dict[str, "CommonCriteriaCert"]:
        ...
        def _parse_table(
            ...
            table_certs = {
                x.dgst: x
                for x in [
                    CommonCriteriaCert.from_html_row(row, cert_status, category_string)
                    for row in body
                ]
            }
    ...
    def _download_reports(self, fresh=True):
        ...
        certs_to_process = [
            x for x in self if x.state.report_is_ok_to_download(fresh) and x.report_link
        ]
    ...
    def download_all_pdfs(self, fresh: bool = True):
        if self.state.meta_sources_parsed is False:
            logger.error(
                "Attempting to download pdfs while not having csv/html meta-sources parsed. Returning."
            )
            return
    ...
    def convert_all_pdfs(self, fresh: bool = True):
        if self.state.pdfs_downloaded is False:
            logger.info(
                "Attempting to convert pdf while not having them downloaded. Returning."
            )
            return
    ...
    def _extract_data(self, fresh: bool = True):
        logger.info(
            "Extracting various stuff from converted txt filed from CC dataset."
        )
    ...
    def _compute_cert_labs(self):
        logger.info(
            "Deriving information about laboratories involved in certification."
        )
    ...
    def _compute_dependencies(self):
        ...
        for dgst in self.certs:
            self.certs[
                dgst
            ].CCHeuristics.directly_affecting = finder.get_directly_affecting(dgst)
            self.certs[
                dgst
            ].CCHeuristics.indirectly_affecting = finder.get_indirectly_affecting(dgst)
            self.certs[
                dgst
            ].CCHeuristics.directly_affected_by = finder.get_directly_affected_by(dgst)
            self.certs[
                dgst
            ].CCHeuristics.indirectly_affected_by = finder.get_indirectly_affected_by(
                dgst
            )
    ...
    def process_maintenance_updates(self):
        maintained_certs: List[CommonCriteriaCert] = [
            x for x in self if x.maintenance_updates
        ]
        updates = list(
            itertools.chain.from_iterable(
                [
                    CommonCriteriaMaintenanceUpdate.get_updates_from_cc_cert(x)
                    for x in maintained_certs
                ]
            )
        )
    ...
    def generate_cert_name_keywords(self) -> Set[str]:
        ...
        keywords = set(
            itertools.chain.from_iterable(
                [x.lower().split(" ") for x in certificate_names]
            )
        )
...
class CCDatasetMaintenanceUpdates(CCDataset, ComplexSerializableType):
    ...
    def to_pandas(self):
        df = pd.DataFrame(
            [x.pandas_tuple for x in self.certs.values()],
            columns=CommonCriteriaMaintenanceUpdate.pandas_columns,
        )
        df = df.set_index("dgst")
        df.index.name = "dgst"

        df.maintenance_date = pd.to_datetime(
            df.maintenance_date, infer_datetime_format=True
        )
        df = df.fillna(value=np.nan)
