...
class CommonCriteriaCert(Certificate, PandasSerializableType, ComplexSerializableType):
    ...
    class MaintenanceReport(ComplexSerializableType):
        ...
        def __post_init__(self):
            ...
            super().__setattr__(
                "maintenance_st_link", helpers.sanitize_link(self.maintenance_st_link)
            )
            super().__setattr__(
                "maintenance_title", helpers.sanitize_string(self.maintenance_title)
            )
            super().__setattr__(
                "maintenance_date", helpers.sanitize_date(self.maintenance_date)
            )
    ...
    class InternalState(ComplexSerializableType):
        ...
        def report_is_ok_to_convert(self, fresh: bool = True):
            return (
                self.report_download_ok
                if fresh
                else self.report_download_ok and not self.report_convert_ok
            )
        ...
        def st_is_ok_to_convert(self, fresh: bool = True):
            return (
                self.st_download_ok
                if fresh
                else self.st_download_ok and not self.st_convert_ok
            )
        ...
        def report_is_ok_to_analyze(self, fresh: bool = True):
            if fresh is True:
                return (
                    ...
                    and self.report_extract_ok
                )
            else:
                return (
                    self.report_download_ok
                    and self.report_convert_ok
                    and not self.report_extract_ok
                )
        ...
        def st_is_ok_to_analyze(self, fresh: bool = True):
            if fresh is True:
                ...
                return (
                    self.st_download_ok
                    and self.st_convert_ok
                    and not self.st_extract_ok
                )
    ...
    class PdfData(ComplexSerializableType):
        ...
        def bsi_data(self) -> Optional[Dict[str, Any]]:
            return (
                self.report_frontpage.get("bsi", None)
                if self.report_frontpage
                else None
            )
        ...
        def anssi_data(self) -> Optional[Dict[str, Any]]:
            return (
                self.report_frontpage.get("anssi", None)
                if self.report_frontpage
                else None
            )
        ...
        @property
        def keywords_rules_cert_id(
            self,
        ) -> Optional[Dict[str, Optional[Dict[str, Dict[str, int]]]]]:
            return (
                self.report_keywords.get("rules_cert_id", None)
        ...
        def cert_id(self) -> Optional[str]:
            return (
                processed
                if (processed := self.processed_cert_id)
                else self.keywords_cert_id
            )
    ...
    def dgst(self) -> str:
        ...
        if not (
            ...
            and self.report_link is not None
            and self.category is not None
        ):
            raise RuntimeError(
                "Certificate digest can't be computed, because information is missing."
            )
        return helpers.get_first_16_bytes_sha256(
            self.category + self.name + self.report_link
        )
    ...
    @classmethod
    def from_html_row(
        cls, row: Tag, status: str, category: str
    ) -> "CommonCriteriaCert":
        """
        ...
        def _get_manufacturer_web(cell: Tag) -> Optional[str]:
            for link in cell.find_all("a"):
                if (
                    link is not None
                    and link.get("title") == "Vendor's web site"
                    and link.get("href") != "http://"
                ):
            ...
            for link in list(cell.find_all("a")):
                if link.get("href") is not None and "/ppfiles/" in link.get("href"):
                    protection_profiles.add(
                        ProtectionProfile(
                            str(link.contents[0]),
                            CommonCriteriaCert.cc_url + link.get("href"),
                        )
                    )
        ...
        def _get_date(cell: Tag) -> Optional[date]:
            text = cell.get_text()
            extracted_date = (
                datetime.strptime(text, "%Y-%m-%d").date() if text else None
            )
        ...
        def _get_maintenance_div(cell: Tag) -> Optional[Tag]:
            ...
            for d in divs:
                if (
                    d.find("div")
                    and d.stripped_strings
                    and list(d.stripped_strings)[0] == "Maintenance Report(s)"
                ):
                    return d
            return None
        ...
        def _get_maintenance_updates(main_div: Tag) -> set:
            ...
            for u in possible_updates:
                ...
                main_date = (
                    datetime.strptime(text.split(" ")[0], "%Y-%m-%d").date()
                    if text
                    else None
                )
                ...
                maintenance_updates.add(
                    CommonCriteriaCert.MaintenanceReport(
                        main_date, main_title, main_report_link, main_st_link
                    )
                )
        ...
        maintenances = (
            _get_maintenance_updates(maintenance_div) if maintenance_div else set()
        )
    ...
    def download_pdf_report(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        ...
        if not cert.report_link:
            ...
            exit_code = helpers.download_file(
                cert.report_link, cert.state.report_pdf_path
            )
        if exit_code != requests.codes.ok:
            error_msg = (
                f"failed to download report from {cert.report_link}, code: {exit_code}"
            )
    ...
    def download_pdf_target(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        ...
        if exit_code != requests.codes.ok:
            error_msg = (
                f"failed to download ST from {cert.report_link}, code: {exit_code}"
            )
            logger.error(f"Cert dgst: {cert.dgst}" + error_msg)
    ...
    def convert_report_pdf(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        exit_code = helpers.convert_pdf_file(
            cert.state.report_pdf_path, cert.state.report_txt_path, ["-raw"]
        )
    ...
    def convert_target_pdf(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        exit_code = helpers.convert_pdf_file(
            cert.state.st_pdf_path, cert.state.st_txt_path, ["-raw"]
        )
        if exit_code != constants.RETURNCODE_OK:
    ...
    def extract_st_pdf_metadata(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        response, cert.pdf_data.st_metadata = helpers.extract_pdf_metadata(
            cert.state.st_pdf_path
        )
    ...
    def extract_report_pdf_metadata(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        response, cert.pdf_data.report_metadata = helpers.extract_pdf_metadata(
            cert.state.report_pdf_path
        )
        if response != constants.RETURNCODE_OK:
    ...
    def extract_st_pdf_frontpage(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        ...
        (
            response_anssi,
            cert.pdf_data.st_frontpage["anssi"],
        ) = helpers.search_only_headers_anssi(cert.state.st_txt_path)
        (
            response_bsi,
            cert.pdf_data.st_frontpage["bsi"],
        ) = helpers.search_only_headers_bsi(cert.state.st_txt_path)
    ...
    @staticmethod
    def extract_report_pdf_frontpage(
        cert: "CommonCriteriaCert",
    ) -> "CommonCriteriaCert":
        cert.pdf_data.report_frontpage = dict()
        (
            response_bsi,
            cert.pdf_data.report_frontpage["bsi"],
        ) = helpers.search_only_headers_bsi(cert.state.report_txt_path)
        (
            response_anssi,
            cert.pdf_data.report_frontpage["anssi"],
        ) = helpers.search_only_headers_anssi(cert.state.report_txt_path)
    ...
    def extract_report_pdf_keywords(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        response, cert.pdf_data.report_keywords = helpers.extract_keywords(
    ...
    def extract_st_pdf_keywords(cert: "CommonCriteriaCert") -> "CommonCriteriaCert":
        response, cert.pdf_data.st_keywords = helpers.extract_keywords(
    ...
    def compute_heuristics_version(self):
        self.heuristics.extracted_versions = helpers.compute_heuristics_version(
            self.name
        )
