...
class FIPSCertificate(Certificate, ComplexSerializableType):
    ...
    def set_local_paths(
        ...
        if fragment_dir is not None:
            self.state.fragment_path = (Path(fragment_dir) / (self.dgst)).with_suffix(
                ".txt"
            )
    ...
    class Algorithm(ComplexSerializableType):
        ...
        def __repr__(self):
            return (
                self.type + " algorithm #" + self.cert_id + " created by " + self.vendor
            )
    ...
    class WebScan(ComplexSerializableType):
        ...
        def __post_init__(self):
            self.date_validation = (
                [parser.parse(x).date() for x in self.date_validation]
                if self.date_validation
                else None
            )
            self.date_sunset = (
                parser.parse(self.date_sunset).date() if self.date_sunset else None
            )
    ...
    def parse_caveat(current_text: str) -> Dict[str, Dict[str, int]]:
        ...
        r_key = (
            r"(?P<word>\w+)?\s?(?:#\s?|Cert\.?(?!.\s)\s?|Certificate\s?)+(?P<id>\d+)"
        )
        for m in re.finditer(r_key, current_text):
            if m.group("word") and m.group("word").lower() in {
                "rsa",
                "shs",
                "dsa",
                "pkcs",
    ...
    def parse_html_main(current_div: Tag, html_items_found: Dict, pairs: Dict):
        ...
        if title in pairs:
            if "date_validation" == pairs[title]:
                ...
                html_items_found["mentioned_certs"].update(
                    FIPSCertificate.parse_caveat(content)
                )
                ...
                html_items_found[
                    "algorithms"
                ] += FIPSCertificate.extract_algorithm_certificates(content)
                ...
                html_items_found[pairs[title]] = [
                    x.text
                    for x in current_div.find("div", class_="col-md-9").find_all("li")
                ]
            else:
                html_items_found[pairs[title]] = content
    ...
    def parse_vendor(current_div: Tag, html_items_found: Dict, current_file: Path):
        ...
        if not vendor_string:
            vendor_string = list(current_div.find("div", "panel-body").children)[
                0
            ].strip()
    ...
    def parse_lab(current_div: Tag, html_items_found: Dict, current_file: Path):
        html_items_found["lab"] = list(current_div.find("div", "panel-body").children)[
            0
        ].strip()
        html_items_found["nvlap_code"] = (
            list(current_div.find("div", "panel-body").children)[2]
            .strip()
            .split("\n")[1]
            .strip()
        )
    ...
    def parse_related_files(current_div: Tag, html_items_found: Dict):
        ...
        html_items_found["security_policy_www"] = constants.FIPS_BASE_URL + links[
            0
        ].get("href")

        if len(links) == 2:
            html_items_found["certificate_www"] = constants.FIPS_BASE_URL + links[
                1
            ].get("href")
    ...
    def html_from_file(
        ...
        file: Path,
        state: State,
        initialized: "FIPSCertificate" = None,
        redo: bool = False,
    ) -> "FIPSCertificate":
        pairs = {
        ...
        return FIPSCertificate(
            ...
            FIPSCertificate.WebScan(
                ...
                if "date_validation" in items_found
                else None,
                ...
                items_found["mentioned_certs"]
                if "mentioned_certs" in items_found
                else None,
                ...
                if "historical_reason" in items_found
                else None,
                items_found["security_policy_www"]
                if "security_policy_www" in items_found
                else None,
                items_found["certificate_www"]
                if "certificate_www" in items_found
                else None,
                ...
                if "revoked_reason" in items_found
                else None,
    ...
    def convert_pdf_file(
        tup: Tuple["FIPSCertificate", Path, Path]
    ) -> "FIPSCertificate":
    ...
    def find_keywords(
    ...
    ) -> Tuple[Optional[Dict], "FIPSCertificate"]:
        if not cert.state.txt_state:
            return None, cert
        ...
        text_to_parse = (
            text_with_newlines if config.use_text_with_newlines_during_parsing else text
        )
        ...
        items_found, fips_text = FIPSCertificate.parse_cert_file(
            FIPSCertificate.remove_platforms(text_to_parse)
        )

        save_modified_cert_file(
            cert.state.fragment_path.with_suffix(".fips.txt"), fips_text, unicode_error
        )
        ...
        save_modified_cert_file(
            cert.state.fragment_path.with_suffix(".common.txt"),
            common_text,
            unicode_error,
        )
    ...
    def match_web_algs_to_pdf(cert: "FIPSCertificate") -> int:
        ...
        if cert.web_scan.algorithms is None:
            raise RuntimeError(
                f"Algorithms were not found for cert {cert.dgst} - this should not be happening."
            )
    ...
    def remove_platforms(text_to_parse: str):
        pat = re.compile(
            r"(?:(?:modification|revision|change) history|version control)\n[\s\S]*?",
            re.IGNORECASE,
        )
            ...
            for rule in search_rules[rule_group]:
                ...
                for m in re.finditer(rule_and_sep, text_to_parse):
                    ...
                    if match_len > MAX_ALLOWED_MATCH_LENGTH:
                        logger.warning(
                            "Excessive match with length of {} detected for rule {}".format(
                                match_len, rule
                            )
                        )
                    ...
                    if sec_certs.constants.APPEND_DETAILED_MATCH_MATCHES:
                        items_found[rule_str][match][
                            constants.TAG_MATCH_MATCHES
                        ].append([match_span[0], match_span[1]])
    ...
    def analyze_tables(
    ...
    ) -> Tuple[bool, "FIPSCertificate", List]:
        cert, precision = tup
        ...
        if tables:
            try:
                data = read_pdf(
                    cert_file, pages="all" if all_pages else tables, silent=True
                )
            ...
            for df in data:
                for col in range(len(df.columns)):
                    if (
                        "cert" in df.columns[col].lower()
                        or "algo" in df.columns[col].lower()
                    ):
    ...
    def _create_alg_set(self) -> Set:
        ...
        if self.web_scan.algorithms is None:
            raise RuntimeError(
                f"Algorithms were not found for cert {self.dgst} - this should not be happening."
            )
    ...
    def remove_algorithms(self):
        ...
        if self.web_scan.mentioned_certs:
            for item, value in self.web_scan.mentioned_certs.items():
                self.heuristics.keywords["rules_cert_id"].update(
                    {"caveat_item": {item: value}}
                )
                ...
                for alg in self.heuristics.keywords["rules_fips_algorithms"]:
                    for found in self.heuristics.keywords["rules_fips_algorithms"][alg]:
                        if (
                            rr.search(found)
                            and rr.search(cert)
                            and rr.search(found).group("id")
                            == rr.search(cert).group("id")
                        ):
                            to_pop.add(cert)

                for alg_cert in self.heuristics.algorithms:
                    for cert_no in alg_cert["Certificate"]:
                        if int("".join(filter(str.isdigit, cert_no))) == int(
                            "".join(filter(str.isdigit, cert))
                        ):
                            to_pop.add(cert)
            for r in to_pop:
    ...
    def compute_heuristics_cpe_vendors(self, cpe_dataset: CPEDataset):
        if self.web_scan.vendor is None:
            raise RuntimeError(
                f"Vendor for cert {self.dgst} not found - this should not be happening."
            )
