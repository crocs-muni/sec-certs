...
def download_parallel(
    items: Sequence[Tuple[str, Path]], num_threads: int
...
def sanitize_date(
    record: Union[pd.Timestamp, date, np.datetime64]
) -> Union[date, None]:
    if pd.isnull(record):
        return None
...
def parse_list_of_tables(txt: str) -> Set[str]:
    ...
    """
    rr = re.compile(
        r"^.+?(?:[Ff]unction|[Aa]lgorithm|[Ss]ecurity [Ff]unctions?).+?(?P<page_num>\d+)$",
        re.MULTILINE,
    )
    pages = set()
...
def find_tables(txt: str, file_name: Path) -> Optional[List]:
    ...
    tables_regex = re.compile(
        r"^(?:(?:[Tt]able\s|[Ll]ist\s)(?:[Oo]f\s))[Tt]ables[\s\S]+?\f", re.MULTILINE
    )
...
def convert_pdf_file(pdf_path: Path, txt_path: Path, options):
    response = subprocess.run(
        ["pdftotext", *options, pdf_path, txt_path],
...
def extract_pdf_metadata(filepath: Path):
    ...
    try:
        ...
        with filepath.open("rb") as handle:
            ...
            metadata.update(
                {key: str(val) for key, val in pdf_document_info.items()}
                if pdf_document_info
                else {}
            )
        ...
        for rule in rules_certificate_preface:
            ...
            for m in re.finditer(rule_and_sep, whole_text):
                ...
                if not other_rule_already_match:
                    ...
                    logger.warning(
                        f"WARNING: multiple rules are matching same certification document: {filepath}"
                    )

                num_rules_hits[rule[1]] += 1  # add hit to this rule
                match_groups = m.groups()
                index_next_item = 0
                items_found[constants.TAG_CERT_ID] = normalize_match_string(
                ...
                if rule[0] == HEADER_TYPE.HEADER_MISSING_CERT_ITEM_VERSION:
                    ...
                    items_found[
                        constants.TAG_CERT_ITEM_VERSION
                    ] = normalize_match_string(match_groups[index_next_item])
                    index_next_item += 1
                ...
                if rule[0] == HEADER_TYPE.HEADER_MISSING_PROTECTION_PROFILES:
                    ...
                    items_found[
                        constants.TAG_REFERENCED_PROTECTION_PROFILES
                    ] = normalize_match_string(match_groups[index_next_item])
                    index_next_item += 1

                items_found[constants.TAG_CC_VERSION] = normalize_match_string(
                    match_groups[index_next_item]
                )
                index_next_item += 1

                items_found[constants.TAG_CC_SECURITY_LEVEL] = normalize_match_string(
                    match_groups[index_next_item]
                )
                index_next_item += 1

                items_found[constants.TAG_DEVELOPER] = normalize_match_string(
                    match_groups[index_next_item]
                )
                index_next_item += 1
            ...
            for m in re.finditer(rule_and_sep, whole_text):
                ...
                for from_keyword in FROM_KEYWORD_LIST:
                    ...
                    if certified_item.find(from_keyword) != -1:
                        ...
                        certified_item_first = certified_item[
                            : certified_item.find(from_keyword)
                        ]
                        developer = certified_item[
                            certified_item.find(from_keyword) + from_keyword_len :
                        ]
        ...
        for rule in rules_certificate_third:
            ...
            for m in re.finditer(rule_and_sep, whole_text):
                ...
                if not constants.TAG_CERT_ID in items_found.keys():
                    logger.error(
                        "ERROR: front page not found for file: {}".format(filepath)
                    )
                ...
                items_found[
                    constants.TAG_REFERENCED_PROTECTION_PROFILES
                ] = normalize_match_string(ref_protection_profiles)
                items_found[constants.TAG_CC_VERSION] = normalize_match_string(
                    cc_version
                )
                items_found[constants.TAG_CC_SECURITY_LEVEL] = normalize_match_string(
...
def extract_keywords(filepath: Path) -> Tuple[str, Optional[Dict[str, Dict[str, int]]]]:
    try:
        result = parse_cert_file(
            filepath, cc_search_rules, -1, sec_certs.constants.LINE_SEPARATOR
        )[0]
        ...
        for key in top_level_keys:
            processed_result[key] = {
                key: val for key, val in gen_dict_extract(result[key])
            }
...
def parse_cert_file(
    file_name, search_rules, limit_max_lines=-1, line_separator=LINE_SEPARATOR
):
    whole_text, whole_text_with_newlines, was_unicode_decode_error = load_cert_file(
        ...
        for rule in search_rules[rule_group]:
            ...
            for m in re.finditer(rule_and_sep, whole_text):
                ...
                if match_len > MAX_ALLOWED_MATCH_LENGTH:
                    print(
                        "WARNING: Excessive match with length of {} detected for rule {}".format(
                            match_len, rule
                        )
                    )
...
def load_cert_html_file(file_name):
    with open(file_name, "r", errors=FILE_ERRORS_STRATEGY) as f:
        try:
            ...
            f.close()
            with open(
                file_name, "r", encoding="utf8", errors=FILE_ERRORS_STRATEGY
            ) as f2:
                try:
                    whole_text = f2.read()
                except UnicodeDecodeError:
...
def compute_heuristics_version(cert_name: str) -> List[str]:
    ...
    matched_strings = set(
        [
            max(x, key=len)
            for x in re.findall(full_regex_string, cert_name, re.IGNORECASE)
        ]
    )
    if not matched_strings:
        matched_strings = set(
            [
                max(x, key=len)
                for x in re.findall(at_least_something, cert_name, re.IGNORECASE)
            ]
        )
