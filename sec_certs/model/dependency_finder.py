...
class DependencyFinder:
    ...
    def _update_direct_references(
        referenced_by: ReferencedByDirect, cert_id: str, this_cert_id: str
    ) -> None:
        ...
        for cert_obj in certificates.values():
            ...
            for cert_id in cert_obj.pdf_data.report_keywords["rules_cert_id"]:
                if cert_id != this_cert_id and this_cert_id is not None:
                    DependencyFinder._update_direct_references(
                        referenced_by, cert_id, this_cert_id
                    )
                ...
                for referencing in tmp_referenced_by_indirect_nums:
                    if referencing in referenced_by.keys():
                        ...
                        newly_discovered_references = [
                            x
                            for x in tmp_referencing
                            if x not in referenced_by_indirect[cert_id]
                        ]
                        referenced_by_indirect[cert_id].update(
                            newly_discovered_references
                        )
                        new_change_detected = (
                            True if newly_discovered_references else False
                        )

        return referenced_by, referenced_by_indirect

    @staticmethod
    def _get_affecting_directly(
        cert: str, referenced_by_direct: ReferencedByDirect
    ) -> Set[str]:
        filter_direct = set()
    ...
    @staticmethod
    def _get_affecting_indirectly(
        cert: str, referenced_by_indirect: ReferencedByIndirect
    ) -> Set[str]:
        filter_indirect = set()
    ...
    @staticmethod
    def _get_affected_directly(
        cert: str, referenced_by_direct: ReferencedByDirect
    ) -> Optional[List[str]]:
        return referenced_by_direct.get(cert, None)

    @staticmethod
    def _get_affected_indirectly(
        cert: str, referenced_by_indirect: ReferencedByIndirect
    ) -> Optional[Set[str]]:
    ...
    def fit(self, certificates: Certificates) -> None:
        ...
        ) = DependencyFinder._build_cert_references(certificates)
        ...
        for dgst in certificates:
            ...
            ] = DependencyFinder._get_affected_indirectly(
            ...
            ] = DependencyFinder._get_affecting_indirectly(
                cert_id, referenced_by_indirect
            )
