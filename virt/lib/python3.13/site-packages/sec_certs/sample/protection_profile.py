...
class ProtectionProfile(
    ...
    class WebData(ComplexSerializableType):
        """
        Class to hold metadata about protection profiles found on commoncriteriaportal.org or NIAP.
        """
        ...
        source: str = "cc_portal"  # "cc_portal", "niap", or "both"
        niap_url: str | None = None
        niap_id: int | None = None
        niap_short_name: str | None = None
        niap_sponsor: str | None = None
        niap_transition_date: date | None = None
        ...
        @classmethod
        def from_niap_dict(cls, data: dict) -> ProtectionProfile.WebData:
            """
            Build WebData from a dict produced by niap_entry_to_pp_dict().
            """
            return cls(
                ...
                not_valid_before=data["not_valid_before"],
                not_valid_after=data["not_valid_after"],
                report_link=data["report_link"],
                pp_link=data["pp_link"],
                scheme=data["scheme"],
                maintenances=data["maintenances"],
                source=data.get("source", "niap"),
                niap_url=data.get("niap_url"),
                niap_id=data.get("niap_id"),
                niap_short_name=data.get("niap_short_name"),
                niap_sponsor=data.get("niap_sponsor"),
                niap_transition_date=data.get("niap_transition_date"),
            )
    ...
    @classmethod
    def from_niap_dict(cls, data: dict) -> ProtectionProfile:
        """
        Builds a `ProtectionProfile` object from a NIAP API dict (processed by niap_entry_to_pp_dict).
        """
        return cls(ProtectionProfile.WebData.from_niap_dict(data))
    ...
    def download_pdf_report(cert: ProtectionProfile) -> ProtectionProfile:
        ...
        if exit_code != requests.codes.ok:
            ...
            filename = unquote_plus(str(urlparse(cert.web_data.report_link).path).split("/")[-1])
            if not filename.lower().endswith(".pdf"):
                filename = cert.dgst + "_report.pdf"
            cert.pdf_data.report_filename = filename
        return cert
    ...
    def download_pdf_pp(cert: ProtectionProfile) -> ProtectionProfile:
        ...
        if exit_code != requests.codes.ok:
            ...
            filename = unquote_plus(str(urlparse(cert.web_data.pp_link).path).split("/")[-1])
            if not filename.lower().endswith(".pdf"):
                filename = cert.dgst + ".pdf"
            cert.pdf_data.pp_filename = filename
        return cert
