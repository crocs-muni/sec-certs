...
class ProtectionProfileDataset(Dataset[ProtectionProfile], ComplexSerializableType):
    ...
    def get_certs_from_web(
        ...
        """
        Fetches list of protection profiles together with metadata from commoncriteriaportal.org and NIAP.
        """
        if to_download:
        ...
        logger.info(f"The dataset has {len(self)} certificates from CC portal.")

        if get_niap:
            niap_certs = self._get_certs_from_niap()
            self._merge_niap_certs(niap_certs)
            logger.info(f"The dataset has {len(self)} certificates after NIAP merge.")
    ...
    def _get_certs_from_niap(self) -> dict[str, ProtectionProfile]:
        """Fetch PPs from the NIAP API and return as dict keyed by digest."""
        from sec_certs.utils.niap import fetch_niap_pp_files, fetch_niap_pps, niap_entry_to_pp_dict

        try:
            niap_entries = fetch_niap_pps()
        except Exception as e:
            logger.error(f"Failed to fetch NIAP PPs: {e}")
            return {}

        niap_certs: dict[str, ProtectionProfile] = {}
        linked = 0
        for entry in niap_entries:
            try:
                pp_id = entry["pp_id"]
                try:
                    files = fetch_niap_pp_files(pp_id)
                except Exception as e:
                    logger.warning(f"Failed to fetch files for NIAP PP {pp_id}: {e}")
                    files = None
                pp_dict = niap_entry_to_pp_dict(entry, files=files)
                if pp_dict.get("pp_link"):
                    linked += 1
                pp = ProtectionProfile.from_niap_dict(pp_dict)
                niap_certs[pp.dgst] = pp
            except Exception as e:
                logger.error(f"Error creating ProtectionProfile from NIAP entry {entry.get('pp_name', '?')}: {e}")
        logger.info(f"Parsed {len(niap_certs)} protection profiles from NIAP ({linked} with PP download link).")
        return niap_certs

    def _merge_niap_certs(self, niap_certs: dict[str, ProtectionProfile]) -> None:
        """
        Merge NIAP PPs into the existing (CC portal) dataset.

        - If a PP with the same digest exists in both, mark it as source="both" and
          add NIAP-specific metadata.
        - If a NIAP PP is not found by digest, try fuzzy name matching.
        - If no match is found, add the NIAP PP as new.
        """
        from rapidfuzz import fuzz

        cc_name_index: dict[str, str] = {}
        for dgst, cert in self.certs.items():
            cc_name_index[cert.web_data.name.lower().strip()] = dgst

        matched = 0
        added = 0
        for niap_dgst, niap_cert in niap_certs.items():
            # Try exact digest match first
            if niap_dgst in self.certs:
                self._enrich_with_niap(self.certs[niap_dgst], niap_cert)
                matched += 1
                continue

            # Try fuzzy name match
            niap_name = niap_cert.web_data.name.lower().strip()
            best_match_dgst = None
            best_score = 0.0
            for cc_name, cc_dgst in cc_name_index.items():
                score = fuzz.token_sort_ratio(niap_name, cc_name)
                if score > best_score:
                    best_score = score
                    best_match_dgst = cc_dgst

            if best_score >= 90 and best_match_dgst is not None:
                self._enrich_with_niap(self.certs[best_match_dgst], niap_cert)
                matched += 1
            else:
                # Add as new NIAP-only PP
                self.certs[niap_dgst] = niap_cert
                added += 1

        logger.info(f"NIAP merge: {matched} matched with CC portal, {added} added as NIAP-only.")

    @staticmethod
    def _enrich_with_niap(cc_cert: ProtectionProfile, niap_cert: ProtectionProfile) -> None:
        """Enrich an existing CC portal PP with NIAP metadata."""
        cc_cert.web_data.source = "both"
        cc_cert.web_data.niap_url = niap_cert.web_data.niap_url
        cc_cert.web_data.niap_id = niap_cert.web_data.niap_id
        cc_cert.web_data.niap_short_name = niap_cert.web_data.niap_short_name
        cc_cert.web_data.niap_sponsor = niap_cert.web_data.niap_sponsor
        cc_cert.web_data.niap_transition_date = niap_cert.web_data.niap_transition_date
        # Use NIAP pp_link as fallback if CC portal didn't provide one
        if not cc_cert.web_data.pp_link and niap_cert.web_data.pp_link:
            cc_cert.web_data.pp_link = niap_cert.web_data.pp_link
